...
*/
    ...
    enum { DEFAULT_KEY_LENGTH = 32 };      // default key length
...
public:
     ...
     * @param keylength: The key length of this AES (16, 24 or 32 bytes, 32 bytes by default).
     */
     ...
     */
//  使用CBC、CFB 和 OFB模式，需要一个向量iv，可增加加密算法的强度，让相同的原文加密出不同的密文
//  IV不需要保密，它可以以明文形式与密文一起传送。IV值必须选取事先无法预测的随机数值，大小和AES
//	分组大小相同。IVs的选取不可以使用计数器方式或低汉明距的数据源，以防止AES-CBC加密多个数据包中
//	存在的非常相似的明文。
     ...
     */
//	计算存储密文所需长度，如原文长度不是16的整数倍，补为16的整数倍。最后加1，为了存放字符串结束符'\0'。
